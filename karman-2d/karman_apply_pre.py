...
#
# Apply correction network model (PRE versions)
#
# ----------------------------------------------------------------------------
...
if params['initvH']:
    vn = downVel(read_zipped_array(params['initvH']))  # read_zipped_array() reverts indices! [...,0]=u, [...,1]=v -> [...,0]=v, [...,1]=u
...
log.info(data_stats)
model = keras.models.load_model(params['model'])  # NOTE: Fully convolutional, so we can use trained weights regardless the input dimension
...
for i in range(1, params['simsteps']):
    ...
    dmani.standardize(idata=inputf, odata=None, dstats=data_stats, sigma_range=data_stats['nsigma'], zero_centered=(False if ('nozerocen' in data_stats) and data_stats['nozerocen'] else True))
    cv_pred = model.predict(inputf)
    dmani.deStandardize(idata=None, odata=cv_pred, dstats=data_stats, sigma_range=data_stats['nsigma'], zero_centered=(False if ('nozerocen' in data_stats) and data_stats['nozerocen'] else True))
